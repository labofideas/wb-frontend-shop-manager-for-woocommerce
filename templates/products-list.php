...
<div class="wbfsm-card">
	...
	<form method="get" class="wbfsm-filters">
		...
		<select name="category">
			...
			<?php foreach ( $wbfsm_terms as $wbfsm_term ) : ?>
				<option value="<?php echo esc_attr( (string) $wbfsm_term->term_id ); ?>" <?php selected( (int) ( $category ?? 0 ), (int) $wbfsm_term->term_id ); ?>><?php echo esc_html( $wbfsm_term->name ); ?></option>
			<?php endforeach; ?>
		</select>
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="wbfsm-bulk-form">
			...
			<label>
				...
				<select name="bulk_status">
					...
					<?php foreach ( WB_FSM_Helpers::product_statuses() as $wbfsm_status_key => $wbfsm_status_label ) : ?>
						<option value="<?php echo esc_attr( $wbfsm_status_key ); ?>"><?php echo esc_html( $wbfsm_status_label ); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
			<label>
		...
		<table class="wbfsm-table">
			...
			<tbody>
				...
				<?php else : ?>
					<?php foreach ( $products as $wbfsm_product ) : ?>
						<?php
						$wbfsm_status_key   = (string) $wbfsm_product->get_status();
						$wbfsm_status_label = WB_FSM_Helpers::product_statuses()[ $wbfsm_status_key ] ?? ucfirst( $wbfsm_status_key );
						$wbfsm_is_variable  = $wbfsm_product->is_type( 'variable' );
						?>
						<tr>
							<td><input type="checkbox" name="product_ids[]" value="<?php echo esc_attr( (string) $wbfsm_product->get_id() ); ?>" class="wbfsm-select-product" /></td>
							<td><?php echo wp_kses_post( $wbfsm_product->get_image( array( 50, 50 ) ) ); ?></td>
							<td><?php echo esc_html( $wbfsm_product->get_name() ); ?></td>
							<td><?php echo esc_html( $wbfsm_is_variable ? __( 'Variable', 'wb-frontend-shop-manager-for-woocommerce' ) : __( 'Simple', 'wb-frontend-shop-manager-for-woocommerce' ) ); ?></td>
							<td><?php echo esc_html( (string) $wbfsm_product->get_sku() ); ?></td>
							<td><?php echo wp_kses_post( wc_price( (float) $wbfsm_product->get_price() ) ); ?></td>
							<td>
								<?php
								if ( $wbfsm_is_variable ) {
									printf(
										/* translators: %d: variation count. */
										esc_html__( '%d variations', 'wb-frontend-shop-manager-for-woocommerce' ),
										(int) count( $wbfsm_product->get_children() )
									);
								} else {
									echo esc_html( (string) $wbfsm_product->get_stock_quantity() );
								}
								?>
							</td>
							<td><?php echo esc_html( $wbfsm_status_label ); ?></td>
							<td><a class="wbfsm-btn wbfsm-btn-secondary wbfsm-btn-sm" href="<?php echo esc_url( add_query_arg( array( 'wbfsm_tab' => 'products', 'product_id' => $wbfsm_product->get_id() ) ) ); ?>"><?php esc_html_e( 'Edit', 'wb-frontend-shop-manager-for-woocommerce' ); ?></a></td>
						</tr>
					<?php endforeach; ?>
				<?php endif; ?>
			</tbody>
