	...
	<form method="get" class="wbfsm-filters">
		...
		<select name="order_status">
			...
			<?php foreach ( wc_get_order_statuses() as $status_key => $status_label ) : ?>
				...
				<option value="<?php echo esc_attr( $status_slug ); ?>" <?php selected( (string) ( $status ?? '' ), $status_slug ); ?>><?php echo esc_html( $status_label ); ?></option>
			<?php endforeach; ?>
		</select>
			...
			<?php else : ?>
				<?php foreach ( $orders as $order ) : ?>
					<tr>
						...
						<td><?php echo esc_html( $order->get_date_created() ? $order->get_date_created()->date_i18n( wc_date_format() ) : '' ); ?></td>
						<td><?php echo esc_html( (string) $order->get_formatted_billing_full_name() ); ?></td>
						<td><?php echo esc_html( wc_get_order_status_name( $order->get_status() ) ); ?></td>
						<td><?php echo wp_kses_post( $order->get_formatted_order_total() ); ?></td>
						<td><a class="wbfsm-btn wbfsm-btn-secondary wbfsm-btn-sm" href="<?php echo esc_url( add_query_arg( array( 'wbfsm_tab' => 'orders', 'order_id' => $order->get_id() ) ) ); ?>"><?php esc_html_e( 'View', 'wb-frontend-shop-manager-for-woocommerce' ); ?></a></td>
					</tr>
				<?php endforeach; ?>
			<?php endif; ?>
		</tbody>
